...
;var VisualSearch = (function($, _){
	...
	VS.ParameterView = Backbone.View.extend({
		...
		autocomplete: function(target, options){
			target.catcomplete(options).catcomplete('widget').addClass('VS-interface');
			return target;
		},
		keydown : function(e) {
	...
	VS.SearchQuery = Backbone.Collection.extend({
		...
		initialize: function(models, options){
			...
			for( var i in this.parameters.key){
				var name = this.parameters.category[i];
				
				this.parameters.list.push({ label: this.parameters.key[i], category: typeof name == "undefined" ? "" : name});
			}
		},
		getEditing: function(){
